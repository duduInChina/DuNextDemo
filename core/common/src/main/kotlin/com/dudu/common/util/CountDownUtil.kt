 ...
 */
open class CountDownUtil @JvmOverloads constructor(
    ...
    constructor(
        ...
        unit: TimeUnit,
        initialDelay: Long = 0
    ) : this(0,-1, period, unit, initialDelay)

    private val subscribeList: MutableList<CountDownUtil.(Long) -> Unit> = mutableListOf()
    private val finishList: MutableList<CountDownUtil.(Long) -> Unit> = mutableListOf()
    ...
    /** 轮询器当前状态 */
    var state = CountDownStatus.STATE_IDLE
        private set
     ...
     */
    fun subscribe(block: CountDownUtil.(Long) -> Unit) = apply {
     ...
     */
    fun finish(block: CountDownUtil.(Long) -> Unit) = apply {
     ...
     */
    fun start() = apply {
        if (state == CountDownStatus.STATE_ACTIVE) return this
        state = CountDownStatus.STATE_ACTIVE
        count = start
        launch()
    }
     ...
     */
    fun stop() {
        if (state == CountDownStatus.STATE_IDLE) return
        scope?.cancel()
        state = CountDownStatus.STATE_IDLE
        finishList.forEach {
    ...
    fun switch() {
        when (state) {
            CountDownStatus.STATE_ACTIVE -> stop()
            CountDownStatus.STATE_IDLE -> start()
            CountDownStatus.STATE_PAUSE -> resume()
        }
    }
     ...
     */
    fun pause() {
        if (state != CountDownStatus.STATE_ACTIVE) return
        scope?.cancel()
        state = CountDownStatus.STATE_PAUSE
     ...
     */
    fun resume() {
        if (state != CountDownStatus.STATE_PAUSE) return
        state = CountDownStatus.STATE_ACTIVE
        launch(delay)
    }
    ...
    fun reset() {
        ...
        scope?.cancel()
        if (state == CountDownStatus.STATE_ACTIVE) launch()
    }
        ...
        scope?.launch {
            ...
            for (unit in ticker) {
                subscribeList.forEach {
                    it.invoke(this@CountDownUtil, count)
                }
                if (end != -1L && count == end) {
                    scope?.cancel()
                    state = CountDownStatus.STATE_IDLE
